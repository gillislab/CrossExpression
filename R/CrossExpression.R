...
#' If one matrix is provided, the output is the pairwise correlations
#' between its columns.
#' If two matrices are provided, the output is the pairwise correlations
...
correlation <- function(matrix1, matrix2 = NULL) {
    ...
    if (is.null(matrix2)) {
        ...
        # correlation with columns of two matrices
...
#' @param binarize If TRUE, converts matrices to binary format to count the
#' number of common elements.
#'
#' @return Returns a column by column matrix, where entries represent the
#' number of common elements.
#' @import Matrix
#'
#' @examples
...
#' stats <- get_cooccurrence_stats(mat1, mat2)
#' stats$cooccur
#' stats$cells
get_cooccurrence_stats <- function(
    matrix1,
    matrix2,
    sparse = TRUE,
    binarize = TRUE
    ...
    # return output
    output <- list(cooccur, cells)
    names(output) <- c("cooccur", "cells")
    return(output)
}
...
#' x <- c(1, 2, 3)
#' y <- c(0, 0, 0)
#' rotate_coordinates(x, y, n_degrees = 90)
...
#'
#' @return Returns a list containing gene pairs with co-expression and
#' cross-expression p-values before and after false discovery rate (FDR)
#' correction.
#' @import Matrix
#' @import Rfast
#' @export
...
cross_expression <- function(
    ...
    # number of shared genes between cell-neighbor pairs (cross-expression)
    # and within the same cells (co-expression)
    hyper_matrices <- get_cooccurrence_stats(
        data,
        data_temp,
        sparse = FALSE,
        binarize = FALSE
    ...
    hyper_matrices$cells <- hyper_matrices$cells[
        order(rownames(hyper_matrices$cells)),
        order(colnames(hyper_matrices$cells))
    ]
    ...
    co_expression <- matrix(
        pval,
        nrow = nrow(hyper_matrices$cells),
        byrow = TRUE,
        dimnames = dimnames(hyper_matrices$cells)
    )
    ...
    hyper_matrices$cooccur <- hyper_matrices$cooccur[
        order(rownames(hyper_matrices$cooccur)),
        order(colnames(hyper_matrices$cooccur))
    ]
    ...
    q <- as.vector(q) - 1
    names(q) <- rep(
        rownames(hyper_matrices$cells),
        times = nrow(hyper_matrices$cells)
    )
    pval <- phyper(q = q, m = m, n = n, k = k, lower.tail = FALSE)
    pvalx <- pval # w/o FDR correction
    pval <- p.adjust(pval, method = "BH")

    cross_expression <- matrix(
        pval,
        nrow = nrow(hyper_matrices$cells),
        byrow = TRUE,
        dimnames = dimnames(hyper_matrices$cells)
    ...
    # assemble output with gene pairs and corresponding p-values for
    # co-expression and cross-expression
    ...
    if (output_matrix) {
        ...
        output_mat <- matrix(
            0,
            nrow = nrow(co_expression),
            ncol = ncol(co_expression)
        )
...
#' @param output_matrix If TRUE, outputs the cross-expression correlation
#' matrix.
#'
#' @return Returns a gene list with cross-expression correlation for each
#' gene pair.
#' @export
#'
#' @examples
...
#'  output_matrix = FALSE
#' )
cross_expression_correlation <- function(
    data,
    locations,
    neighbor = 1,
    output_matrix = FALSE
    ...
    corr <- data.frame(
        gene1 = rownames(corr)[ids[, 1]],
        gene2 = colnames(corr)[ids[, 2]],
        correlation = upper_tri(corr)
    )
...
#' Smooths cells' gene expression by averaging its expression by the
#' nearest neighbors.
#' Optionally computes genes by genes Pearson's correlation matrix
#' between cells by genes
#' and neighbors by genes matrices.
#'
#' @param data A cells by genes expression matrix.
#' @param locations A cells by coordinates (x-y or higher dimensions)
#' matrix.
#' @param neighbors_smoothed Numbers of neighbors used for smoothing
#' (0 is the cell itself; 1 is the nearest neighbor).
#' @param corr If TRUE, computes genes by genes correlation matrix between
#' regions.
#'
#' @return Returns a smoothed gene expression matrix. If corr = TRUE,
#' additionally returns a gene-gene correlation matrix.
#' @export
#'
#' @examples
#' data(expression)
...
smooth_cells <- function(data, locations, neighbors_smoothed = 1, corr = TRUE) {
    ...
    ids <- cbind(
        rep(seq_len(nrow(locations)), neighbors_smoothed),
        as.vector(distances)
    )
    cellbycell[ids] <- 1
    ...
    # gene x gene correlation between smoothed cells x genes matrix and
    # smoothed neighbors x genes matrix 'neighbors' are regions at n + 1 cells
    if (corr) {
        ...
        output <- list(data_smooth, corr)
        names(output) <- c("smooth_expression", "smooth_correlation")
...
#'
#' @return Returns a dataframe with each row representing a gene pair and
...
#'  output_matrix = FALSE
#' )
bullseye_scores <- function(
    data,
    locations,
    window_sizes = seq_len(10),
    ratio_to_co = FALSE,
    log_2 = FALSE,
    output_matrix = FALSE
) {
    ...
    outcome <- matrix(
        data = 0,
        ncol = length(c(0, window_sizes)),
        nrow = choose(ncol(data), 2) * 2
    )
    colnames(outcome) <- c("Cell", str_c("Neig = ", window_sizes))

    # compute scores
    for (i in seq_len(ncol(outcome))[-1]) {
    ...
    if (log_2) {
        if (ratio_to_co) {
            outcome <- log2(outcome)
        }
        if (!ratio_to_co) {
            outcome <- log2(outcome + 1)
        }
    }
    ...
    if (output_matrix) {
        # storage list
        outcome_matrix <- vector(
            mode = "list",
            length = length(c(1, window_sizes))
        )
        ...
        upper_triangle_indices <- which(
            upper.tri(reconstructed_data),
            arr.ind = TRUE
        )
        lower_triangle_indices <- which(
            upper.tri(t(reconstructed_data)),
            arr.ind = TRUE
        )

        # create matrices
        for (i in 3:ncol(outcome)) {
            reconstructed_data[upper_triangle_indices] <- outcome[, i][
                seq_len(length(upper_triangle_indices[, 1]))
            ]
            reconstructed_data <- t(reconstructed_data)
            reconstructed_data[lower_triangle_indices] <- outcome[, i][
                (length(upper_triangle_indices[, 1]) + 1):nrow(outcome)
            ]
            reconstructed_data <- t(reconstructed_data)

            rownames(reconstructed_data) <- colnames(data)
            colnames(reconstructed_data) <- colnames(data)
...
#' Rings indicate neighbors with gene B, where the first ring is the first
#' neighbor.
#'
...
#'     bull[bull$gene1 %in% "Galntl6" & bull$gene2 %in% "Nrg1", 3:ncol(bull)]
#' )
#' bullseye_plot(scores = score_vector)
bullseye_plot <- function(scores) {
    # initialization
    vec_values <- as.numeric(scores)
    window_sizes <- seq_len(length(vec_values))
    radius_ratio <- 1
    ...
    for (i in seq_len(n_circles)[-1]) {
        radii[i] <- radii[i - 1] + initial_radius * radius_ratio^(i - 1)
    }
    ...
    for (i in seq_len(length(radii))) {
        ...
        df_circle <- data.frame(
            x = c(cos(theta) * inner_radius, rev(cos(theta) * outer_radius)),
            y = c(sin(theta) * inner_radius, rev(sin(theta) * outer_radius)),
            id = rep(i, length(theta) * 2),
            vec_value = rep(vec_values[i], length(theta) * 2)
        )
        df_list[[i]] <- df_circle
    }

    df <- bind_rows(df_list)
    colnames(df) <- c("x", "y", "window", "hue")

    # create bullseye plot
    p <- ggplot(df, aes(x = .data$x, y = .data$y, fill = .data$hue)) +
        geom_polygon(aes(group = window)) +
        coord_fixed(ratio = 1) +
...
#' @param max_pairs Specify maximum number of cell pairs to consider. Lower
#' number increases computational efficiency.
#'
#' @return Returns a p-value and distance distributions between cross-expressing
...
spatial_enrichment <- function(
    ...
    # average using locations of neighboring cells and append cross-expressing
    # pairs
    ...
    locations1 <- locations[locations$cross == 1, ]
    locations1 <- locations1[, c(1, 2)]
    locations1 <- locations1[
        sample(
            seq_len(nrow(locations1)),
            size = min(c(max_pairs, nrow(locations1))),
            replace = FALSE
        ),
    ]

    locations0 <- locations[locations$cross == 0, ]
    locations0 <- locations0[, c(1, 2)]
    locations0 <- locations0[
        sample(
            seq_len(nrow(locations0)),
            size = nrow(locations1),
            replace = FALSE
        ),
    ]
    ...
    pp <- data.frame(
        vals = c(target, null),
        type = rep(
            c("Cross-expressing", "Random"),
            times = c(length(target), length(null))
        )
    )
    pp$type <- factor(pp$type, levels = c("Random", "Cross-expressing"))
    pp <- ggplot(pp) +
        aes(x = .data$vals, fill = .data$type, y = after_stat(.data$scaled)) +
        geom_density(alpha = 0.8) +
        labs(x = "Distance to cells", y = "Density", fill = "") +
        theme_classic() +
        guides(fill = guide_legend(reverse = TRUE)) +
        scale_fill_manual(
            values = c("Cross-expressing" = "lightblue", "Random" = "gray88")
        ) +
        theme(
            legend.position = "top",
...
#'
#' @return Returns a plot with cells shown as points and color indicating the
#' genes it expresses.
#' @import Matrix
#' @import ggplot2
#' @importFrom rlang .data
...
#'  gene2 = "Col19a1"
#' )
tissue_expression_plot <- function(
    data,
    locations,
    gene1,
    gene2,
    ...
    if (!cross_expression) {
        ...
        locations$type <- factor(
            locations$type,
            levels = c("Neither", "Both", "Gene2", "Gene1")
        )
        locations <- locations[order(locations$type), ]

        p <- ggplot(locations) +
            aes(x = .data$x, y = .data$y, color = type) +
    ...
    p <- ggplot(meta_reordered) +
        aes(x = .data$x, y = .data$y, color = light) +
        geom_point(size = point_size) +
        scale_color_manual(
            values = c(
                "Neither" = "gray88",
