...
```{r}
corr <- cross_expression_correlation(
    data = expression,
    locations = locations,
    neighbor = 1,
    output_matrix = FALSE
...
smth <- smooth_cells(
    data = expression,
    locations = locations, neighbors_smoothed = 5, corr = TRUE
)
```
...
```{r}
# rotate coordinates
locations2 <- rotate_coordinates(
    x = locations_df$pos_x,
    y = locations_df$pos_y,
    n_degrees = 20,
    center = TRUE,
    scale = TRUE,
    flip_x = TRUE,
